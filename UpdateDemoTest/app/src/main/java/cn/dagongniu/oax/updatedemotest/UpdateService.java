...
/**
 * Created by Sun on 服务更新 2016/6/12.
 */
...
//        Uri uri = Uri.fromFile(new File(path));
    ...
    private class DownloadApk extends AsyncTask<String, Integer, Uri> {
        ...
        protected Uri doInBackground(String... params) {
            ...
            File file = UpdateService.getDownloadDir(updateServiceWeakReference.get());
//            final File file = new File(file, UpdateService.getSaveFileName(downloadUrl));
            if (DEBUG) {
            ...
            try {
                ...
                if (file.exists()) {
                    if (updateTotalSize == file.length()) {
                        // 下载完成
                        return FileProvider.getUriForFile(MyApplication.getInstance(), "com.hfcai.fileprovider", file);
                    } else {
                        file.delete();
                    }
                }
